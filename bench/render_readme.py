...
def table(t: dict) -> str:
    ...
    lines = [
        "| configuration | " + " | ".join(f"B = {b:,}" for b in batches) + " |",
    ...
    for name, label in ROWS:
        if name not in series:
            continue
        cells = [
            fmt(series[name][b], stock, b, name == "stock_eager")
            if b in series[name]
            else "—"
            for b in batches
        ]
...
def main(results_dir: str) -> None:
    ...
    subprocess.run(
        [sys.executable, str(HERE / "plot_bench.py"), str(d), "--out-dir", str(HERE)],
        check=True,
    )
    ...
    parts.append("")
    parts.append(
        f"Wall time of one `LLM.generate()` call ({big}, speedup vs stock in bold):"
    )
    parts.append("")
    ...
    parts.append(
        ...
        "is two waves there -- see the vLLM caveat below (packed GDN decode kernel grid limit). Decode at these "
        "sizes is compute-bound and scales linearly (2,048 takes 2x the 1,024 time in every configuration)."
    )
    for m in models:
        if m == big:
            continue
        ...
        parts.append("")
        parts.append(
            f"Second panel, {m} (same protocol; small models are hook-overhead dominated, so the gap is larger):"
        )
        parts.append("")
    ...
    parts.append("")
    parts.append(
        "Full numbers, per-condition hook counters and every correctness assertion: `bench/results/` "
    ...
    a, b = (
        readme.index("<!-- DIFFSTAT:BEGIN -->"),
        readme.index("<!-- DIFFSTAT:END -->"),
    )
    readme = (
        readme[: a + len("<!-- DIFFSTAT:BEGIN -->")]
        + "\n```\n"
        + stat
        + "\n```\n"
        + readme[b:]
    ...
    shutil.copy(d / "summary.json", HERE / "results_summary.json") if (
        d / "summary.json"
    ).exists() else None
